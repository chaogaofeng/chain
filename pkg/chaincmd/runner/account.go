...
// AddGenesisPermission adds account roles to genesis by its address.
func (r Runner) AddGenesisAccountRoles(ctx context.Context, address string, roles []string) error {
	return r.run(ctx, runOptions{}, r.chainCmd.AddGenesisAccountRolesCommand(address, roles))
}
